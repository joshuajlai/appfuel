...
/**
 */
class QueryHandler implements MysqliQueryHandlerInterface
{
	/**
	 * @var	mysqli
	 */
	...
	public function execute(DbRequestInterface $request)
	{
		...
		switch($type) {
			case 'query':		  return $this->query($request);		 break;
			case 'multi-query':	  return $this->multiQuery($request);    break;
			case 'prepared-stmt': return $this->preparedQuery($request); break;
